...
var key="2738907fc1f77d1722caebb3fc986ccc";
	...
	$.ajax({
		...
		success: function(res){
			...
			res.tracks.track.forEach(function(item, index){
				...
				html += '<div class="singer">'+ item.artist.name +'</div>';
				html += '<img src="../static/img/ic_play_green@2x.png" class="playBtn"/></div>';
	...
	$.ajax({
		...
		success: function(res){
			...
			res.tracks.track.forEach(function(item, index){
				...
				html += '<div><img src="../static/img/ic_plus@2x.png" class="addBtn"/></div>';
